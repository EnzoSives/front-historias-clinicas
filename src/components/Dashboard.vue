              ...
              >
                ...
                <q-item-section side top>
                  <div class="text-caption text-grey-6">
                    {{ formatRelativeDate(patient.fechaCreacion) }}
                  </div>
                  <q-chip
...
const consultationsThisMonth = computed(
  () => medicalStore.getConsultationsThisMonth
);
...
const avgConsultationsPerDay = computed(() => {
  ...
  return consultationsThisMonth.value > 0
    ? Math.round((consultationsThisMonth.value / daysInMonth) * 10) / 10
    : 0;
});
...
const getConsultationStatus = (consultation: ConsultationType): string => {
  ...
  const daysDiff = Math.floor(
    (new Date().getTime() - consultationDate.getTime()) / (1000 * 60 * 60 * 24)
  );
