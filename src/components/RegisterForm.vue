  ...
  <q-card class="q-ma-md" style="max-width: 800px; width: 100%">
    ...
    <q-card-section>
      <q-form @submit="handleSubmit" class="q-gutter-md">
        ...
        <q-input v-model="form.username" label="Username" filled :rules="[
          (val) => !!val || 'El username es requerido',
          (val) =>
            (val.trim().length >= 3 && val.trim().length <= 50) ||
            'El username debe tener entre 3 y 50 caracteres',
        ]" />
        <q-input v-model="form.email" label="Email" filled type="email" :rules="[
          (val) => !!val || 'El email es requerido',
          (val) => /.+@.+\..+/.test(val) || 'Debe ser un email válido',
        ]" />
        <q-input v-model="form.password" label="Contraseña" filled type="password" :rules="[
        ...
        <div class="row q-gutter-md">
          <q-input v-model="form.nombre" label="Nombre" filled class="col" :rules="[
            (val) => !!val || 'El nombre es requerido',
            (val) =>
              (val.trim().length >= 2 && val.trim().length <= 100) ||
              'El nombre debe tener entre 2 y 100 caracteres',
          ]" />
          <q-input v-model="form.apellido" label="Apellido" filled class="col" :rules="[
        ...
        <div class="row q-gutter-md">
          <q-input v-model="form.dni" label="DNI" filled class="col" :rules="[
            (val) => !!val || 'El DNI es requerido',
            (val) =>
              /^\d{7,8}$/.test(val) || 'El DNI debe tener 7 u 8 dígitos',
          ]" />
          <q-input v-model="form.especialidad" label="Especialidad" filled class="col" :rules="[
        ...
        <q-input v-model="form.matricula" label="Matrícula" filled :rules="[
        ...
        <q-input v-model="form.telefono" label="Teléfono" filled hint="Opcional" :rules="[
          (val) =>
            !val ||
            /^[\d\s\-\+\(\)]+$/.test(val) ||
            'El teléfono tiene un formato inválido',
        ]" />
        <q-input v-model="form.colegioMedico" label="Colegio Médico" filled hint="Opcional" :rules="[
          (val) =>
            !val ||
            (val.trim().length >= 2 && val.trim().length <= 200) ||
