  ...
  actions: {
    ...
    async fetchAllPatients() {
      ...
      try {
        const response = await api.get<PatientType[]>(
          "http://localhost:3000/paciente/all"
        ); //
    ...
    async addPatient(patientData: Partial<PatientType>) {
      ...
      try {
        const response = await api.post<PatientType>(
          "http://localhost:3003/paciente/crear",
          patientData
        ); //
        ...
        this.error =
          err.response?.data?.message || "Error al añadir el paciente.";
    ...
    async updatePatient(id: number, patientData: Partial<PatientType>) {
      ...
      try {
        const response = await api.patch<PatientType>(
          `http://localhost:3000/paciente/actualizar/${id}`,
          patientData
        ); //
    ...
    async deletePatient(id: number) {
      ...
      try {
        ...
        this.error =
          err.response?.data?.message || "Error al eliminar el paciente.";
        throw err;
      } finally {
        this.loading = false;
      }
    ...
    async fetchPatientById(id: number) {
      ...
      try {
        ...
        this.error =
          ...
          `Error al cargar el paciente con ID ${id}.`;
        throw err;
      } finally {
        this.loading = false;
      }
